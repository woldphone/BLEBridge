...
VERSION = "1.1.0"
REPO_RAW_URL = "https://raw.githubusercontent.com/woldphone/BLEBridge/main/"

# --- Bootstrap Dependencies & Auto-Update ---
def bootstrap():
    required_packages = ["textual", "rich", "requests"]
    print(f"[*] BLE Bridge TUI v{VERSION}")
    ...
    try:
        ...
        response = requests.get(REPO_RAW_URL + "ble_tui.py", timeout=5)
        if response.status_code == 200:
            ...
            if remote_version and remote_version != VERSION:
                print(f"[+] New version found: {remote_version}. Updating...")
                with open(__file__, "w") as f:
                    f.write(content)

                # Also update uuids.json
                res_uuids = requests.get(REPO_RAW_URL + "uuids.json", timeout=5)
                if res_uuids.status_code == 200:
                    with open("uuids.json", "w") as f:
                        f.write(res_uuids.text)
